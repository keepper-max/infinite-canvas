...
- 项目已有 ECS 运行基线和旧 PostgreSQL/IndexedDB 数据；涉及云端权威数据时必须使用增量迁移并保留回滚，不能用空库或新结构直接覆盖旧数据。
...
- 外部服务请求统一放在 `web/src/services/api/`；账号、项目、画布、资产和任务走平台 API，内置模型走服务端模型代理，只有用户自定义渠道可按其配置直连。
...
- 项目壳和画布结构已由 PostgreSQL 权威保存；云端资产与不可变版本正在替代本地素材，本地 IndexedDB 仅作为离线缓存和显式迁移来源。
- 内置模型 API Key 只允许由服务器环境变量注入；用户自定义渠道仍可能保存在浏览器配置中，安全说明必须区分两条链路。
