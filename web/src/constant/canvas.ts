...
export function getNodeSpec(type: string) {
    if ((Object.values(CanvasNodeType) as string[]).includes(type)) return NODE_SPECS[type as CanvasNodeType] as CanvasNodeSpec;
    const spec = getRegistryNodeSpec(type);
    return { width: spec.width, height: spec.height, title: spec.title, metadata: spec.metadata, definitionVersion: spec.definitionVersion, workflowKind: spec.workflowKind } satisfies CanvasNodeSpec;
}
