                            ...
                            {teams.map((team) => (
                                <div key={team.id} className="rounded-xl border border-stone-200 p-4 dark:border-stone-800">
                                    ...
                                    <div className="mt-2 flex items-center justify-between text-xs text-stone-500">
                                        ...
                                        <Button size="small" onClick={() => void openTeam(team)}>
                                            管理
                                        </Button>
                                    </div>
                                </div>
                ...
                <div className="space-y-5">
                    <div>
                        ...
                        <div className="space-y-2">
                            {members.map((member) => (
                                <div key={member.userId} className="flex justify-between rounded-lg bg-stone-100 px-3 py-2 text-sm dark:bg-stone-900">
                        ...
                        <>
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <Input value={memberEmail} placeholder="已注册邮箱" onChange={(event) => setMemberEmail(event.target.value)} />
                                    <Select
                                        value={memberRole}
                                        className="w-28"
                                        options={[
                                    ...
                                    />
                                    <Button type="primary" onClick={() => void submitMember()}>
                                        保存
                                    </Button>
                                </div>
                            </div>
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <Select
                                        ...
                                        className="flex-1"
                                        placeholder="选择项目"
                                        options={projects.filter((project) => project.role === "owner").map((project) => ({ value: project.projectId, label: project.projectTitle }))}
                                        onChange={setProjectId}
                                    />
                                    <Button onClick={() => void submitProject()}>关联</Button>
                                </div>
                            </div>
                        </>
                    ) : null}
