...
export function usePromptList({ keyword, tags, category, enabled = true }: { keyword: string; tags: string[]; category: string; enabled?: boolean }) {
    ...
    const query = useInfiniteQuery({
        queryKey: ["prompts", user.isAdmin, debouncedKeyword, tags, category],
        queryFn: ({ pageParam }) => fetchPrompts({ keyword: debouncedKeyword, tag: tags, category, page: pageParam, pageSize: PROMPT_PAGE_SIZE, includeRestricted: user.isAdmin }),
        initialPageParam: 1,
