...
import { ensureImagePreview, getImagePreviewRevision, previewUrlFor, subscribeImagePreviews } from "@/services/image-storage";
                    ...
                    >
                        {referenceSelectionState !== "disabled" ? (
                            <span className="rounded-lg px-3 py-2 text-sm font-medium shadow-sm" style={{ background: theme.toolbar.panel, color: theme.node.text }}>
                                {t(referenceSelectionState === "target" ? "canvas.references.cancelSelecting" : "canvas.references.choose")}
                            </span>
                        ) : null}
                    </div>
                ) : null}
...
function ConnectionHandleDot({ side, visible, onMouseDown }: { side: "left" | "right"; visible: boolean; onMouseDown: (event: React.MouseEvent) => void }) {
    ...
    return (
        ...
        >
            <span
                className={`pointer-events-none absolute top-1/2 -translate-y-1/2 whitespace-nowrap rounded px-1.5 py-0.5 text-[10px] opacity-0 shadow-sm transition-opacity group-hover/port:opacity-100 ${side === "left" ? "right-9" : "left-9"}`}
                style={{ background: theme.toolbar.panel, color: theme.node.muted }}
            >
                {side === "left" ? "输入" : "输出"}
            </span>
