...
- 当前任务包：第 6 部分漫剧节点、分镜、角色与 Seedance Skill 已完成。
- 下一动作：等待用户要求提交/推送第 6 部分，或明确执行第 7 部分；不自动跨任务包。
...
- 我在哪里：第 0 至第 6 部分已完成；第 6 部分改动仍在工作区，尚未提交或推送。
- 目标是什么：把当前 Infinite Canvas 推进为可完成一集漫剧的云端生产平台。
- 下一步是什么：用户确认后提交/推送第 6 部分，或明确执行第 7 部分声音、字幕、时间线与 FFmpeg 成片。
- 已知事实在哪里：`findings.md`。
