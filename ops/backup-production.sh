...
docker compose --env-file "$env_file" -f "$compose_file" ps --all > "$backup_dir/containers.txt"
docker compose --env-file "$env_file" -f "$compose_file" images > "$backup_dir/images.txt"
...
docker compose --env-file "$env_file" -f "$compose_file" exec -T db sh -ec \
